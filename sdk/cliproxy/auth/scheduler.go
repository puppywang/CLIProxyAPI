...
func (s *authScheduler) pickSingleWithStrategy(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}, strategy schedulerStrategy) (*Auth, error) {
	...
	predicate := scheduledAuthPredicate(tried, pinnedAuthID, strategy == schedulerStrategyWeightedRoundRobin)
...
func (s *authScheduler) pickMixedWithStrategy(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}, strategy schedulerStrategy) (*Auth, string, error) {
	...
	predicate := scheduledAuthPredicate(tried, "", strategy == schedulerStrategyWeightedRoundRobin)
...
// It excludes auths already attempted, non-pinned auths when a pin is set, and
// (under weighted round-robin) credentials with weight <= 0.
func scheduledAuthPredicate(tried map[string]struct{}, pinnedAuthID string, requirePositiveWeight bool) func(*scheduledAuth) bool {
