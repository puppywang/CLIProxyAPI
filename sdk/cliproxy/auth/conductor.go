...
func (m *Manager) pickNextLegacy(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	// Hand the full admin-filtered candidates straight to the selector.
	// Cooldown filtering lives inside the selectors (getAvailableAuths)
	// so the affinity selector's strict-bypass branch can honour a
	// session binding even when the bound auth is currently in a
	// transient cooldown — that's the whole point of strict-bypass.
	// The conductor's earlier pre-filter (availableAuthsForRouteModel)
	// silently defeated this by stripping cooldown'd auths before the
	// selector ever saw them. Per-auth OAuth alias resolution travels
	// via ctx so the selector's filter uses the same model-key
	// resolution the pre-filter used to do inline.
	pickCtx := WithAliasResolver(ctx, m.selectionModelForAuth)
	selected, errPick := m.selector.Pick(pickCtx, provider, selectionArgForSelector(m.selector, model), opts, candidates)
	if errPick != nil {
		m.mu.RUnlock()
...
func (m *Manager) pickNextMixedLegacy(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	...
	// See the matching block above (single-provider path): hand full
	// candidates to the selector with alias resolver in ctx; cooldown
	// filtering happens inside the selectors so strict-bypass survives.
	pickCtx := WithAliasResolver(ctx, m.selectionModelForAuth)
	selected, errPick := m.selector.Pick(pickCtx, "mixed", selectionArgForSelector(m.selector, model), opts, candidates)
	if errPick != nil {
		m.mu.RUnlock()
		return nil, nil, "", errPick
	}
